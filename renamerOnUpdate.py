...
def callGraphQL(query, variables=None):
    ...
    graphql_url = f"http://localhost:9999/graphql"
#    graphql_url = f"{graphql_scheme}://{graphql_domain}:{graphql_port}/graphql"
    ...
    try:
        response = requests.post(graphql_url, json=json, headers=graphql_headers, cookies=graphql_cookies, timeout=120)
...
def create_new_filename(scene_info: dict, template: str):
    ...
    if FILENAME_TITLECASE:
        ...
        new_filename = capitalizeWords(name) + ext.lower()
...
def checking_duplicate_db(scene_info: dict):
    ...
    if scenes["count"] > 0:
        ...
        for dupl_row in scenes["scenes"]:
            ...
            c.execute("INSERT INTO scenes_tags (scene_id, tag_id) VALUES (?, ?)", (dupl_row['id'], 300))
            log.LogWarning(f"Added Tag '300' to scene: [{dupl_row['id']}]")
        conn.commit()
        conn.close()
        return 1
